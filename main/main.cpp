...
void inc_uuid(uint8_t *uuid) {
	...
	if (carry) { ESP_LOGW(TAG, "uuid overflow"); }
}
...
static void gatts_event_handler(
	...
	case ESP_GATTS_CREATE_EVT: {
		...
		if (param->create.status) {
			ESP_LOGW(TAG, "event %d, status %d, service_handle 0x%02x",
	...
	case ESP_GATTS_ADD_CHAR_EVT: {
		...
		if (param->add_char.status) {
			ESP_LOGW(TAG, "event %d, status %d, attr_handle 0x%02x, "
	...
	case ESP_GATTS_ADD_CHAR_DESCR_EVT: {
		...
		if (param->add_char_descr.status) {
			ESP_LOGW(TAG, "event %d, status %d, attr_handle 0x%0x, "
